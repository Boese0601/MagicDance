...
def get_mask(mask_mode, img_size, **mask_config):
    if mask_mode == 'bbox':
        ...
        mask = brush_stroke_mask(img_size, brush_width=(img_size[0]//6, img_size[1]//3)) #(12,25))
...
if __name__ == "__main__":
    ...
    for i in range(100):
        res = get_mask("free_form", [64,64])
        res = res.repeat(3,1,1) * 255
        res = res.permute(1, 2, 0).type(torch.uint8).numpy()
        print(res.shape)
        cv2.imwrite(f'./results/mask_{i}.png', res)
